 ...
 */
@TestGuiceyApp(App)
class BoundNotExposedPrivateExtensionTest extends AbstractTest {
    ...
    static class Module extends PrivateModule {
        ...
        protected void configure() {
            // visible extension
            bind(Bind).to(Ext)
            // important: Ext not exposed, but binding already exists (no synthetic must be added)
            bind(Ext).toProvider({ new Ext() } as Provider<Ext>)
            expose(Bind)
