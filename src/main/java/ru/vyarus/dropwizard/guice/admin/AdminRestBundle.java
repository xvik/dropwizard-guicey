 ...
 * <p>Such approach is better than registering completely separate jersey context for admin rest because
 * of no overhead and simplicity of jersey extensions management.</p>
 * <p>If no specific mapping path specified, rest will be mapped into the same path as main rest.
 * If main rest mapping default ('/*') isn't changed, admin rest will be mapped to '/api/*'.</p>
 * <p>In order to hide admin specific rest methods or entire resources
 * {@link ru.vyarus.dropwizard.guice.admin.rest.AdminResource} annotation may be used.
 * If some security solution is used within application, rest could be hidden with security framework permissions.</p>
 *
